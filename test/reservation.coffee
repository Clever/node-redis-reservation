...
class BaseWorker
  ...
  _hearbeat_interval: 10*60*1000  # 10 minutes in milliseconds for system reservation
  _lock_ttl: 30*60  # 30 minutes in seconds for system reservation
  _exit_on_throw: true  # process.exit if errors are thrown, or systems are stolen
...
class LockWorker extends BaseWorker
  ...
  _run: (payload, cb) ->
    ...
    @reservation.lock payload.system_id, (err, state) =>
...
class WaitWorker extends BaseWorker
  ...
  _run: (payload, cb) ->
    ...
    @reservation.wait_until_lock payload.system_id, (err, state) =>
      return cb "no_reservations" unless state
      return cb null, 'patience_is_bitter_but_fruit_is_sweet'

describe "coffee worker system locks", ->
  ...
  it 'can reserve and release a lock', (done) ->
    test_worker = new LockWorker system_id: 'test_system', (err, resp) =>
      assert.equal null, err
      assert.equal resp, 'whostheboss.iam'
      @redis.get 'system-test_system', (err, resp) ->
        assert.equal resp, null
        done()

  it 'holds a lock while running', (done) ->
    test_worker = new SlowWorker system_id: 'test_system', (err, resp) =>
      assert.equal null, err
      assert.equal resp, 'whostheboss.iam'
      @redis.get 'system-test_system', (err, resp) ->
        assert.equal resp, null
  ...
  it 'can wait for a lock', (done) ->
    setTimeout(=>
      @redis.del 'system-test_system', -> return
    , 1000)

    @redis.set 'system-test_system', 'MOCK', (err, resp) =>
      test_worker = new WaitWorker system_id: 'test_system', (err, resp) =>
        assert.equal null, err
  ...
  it 'fails silently if system is already reserved', (done) ->
    @redis.set 'system-test_system', 'MOCK', (err, resp) =>
      test_worker = new LockWorker system_id: 'test_system', (err, resp) =>
  ...
  it 'does not interfere for workers without reservations', (done) ->
    @redis.set 'system-test_system', 'MOCK', (err, resp) =>
      test_worker = new FreeWorker system_id: 'test_system', (err, resp) =>
  ...
  it 'handles failing jobs', (done) ->
    @redis.set 'system-test_system', 'MOCK', (err, resp) =>
      test_worker = new FailWorker system_id: 'test_system', (err, resp) =>
